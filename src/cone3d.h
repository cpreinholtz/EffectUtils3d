...
public:
    ...
    float distance(Vector3d position) const {
        ...
        if (coneHeight * pointHeight < 0.0) {//lazy way to tell if the points are on opposite sides of the plane
            ...
            dist = myAbs(pointHeight); //find the distance from plane
        ...
        else if (myAbs(coneHeight) < myAbs(pointHeight)) {//point is "above" cone
            ...
            dist = c2 - mRadias * myAbs(heightRatio); // use that ratio for finding radias at this height
        }
        return dist; 
...
#include <iostream>
    ...
    friend std::ostream& operator<<(std::ostream& os, const Plane3d& dt) {
        os << dt.mRefPoint << dt.mNormalVector << dt.mRadias << dt.mTopPoint;
        return os;
    }
#endif
